...
export default function History() {
  ...
  const [pro, setPro] = useState<string>(team[0]?.name ?? "");
  ...
  return (
    <div>
      ...
      {team.length === 0 ? (
        <Card><p className="text-sm text-muted-foreground italic">Nenhum profissional cadastrado ainda.</p></Card>
      ) : (
        <>
          <div className="flex items-center gap-3 mb-6">
            {proData && <Avatar initials={proData.initials} photoUrl={getPhoto(proData.name)} size="lg" />}
          ...
          {personalHistory.length === 0 ? (
            <Card><p className="text-sm text-muted-foreground italic">Nenhum evento no histórico ainda.</p></Card>
          ) : (
